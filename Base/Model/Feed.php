...
use Awd\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExistsFactory;
...
class Feed
{
     ...
     * Path to NEWS
     */
    const URN_NEWS = 'advancedwebsitedesign.co.uk/feed-news-segments.xml';//do not use https:// or http

    /**
     * Path to ADS
     */
    const URN_ADS = 'advancedwebsitedesign.co.uk/media/marketing/upsells.csv';
        ...
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            ...
            foreach ($feedXml->channel->item as $item) {
                ...
                if ($priority <= $maxPriority
                    ...
                    || !$this->validateByAwdCount($item->awd_module_qty)
                    || !$this->validateByNotInstalled((string)$item->awd_module_not)
     ...
     * @return array|string[]
     */
    private function getInstalledAwdExtensions()
    {
        if (!$this->awdModules) {
            ...
            $modules = array_filter(
                ...
                static function ($item) {
                    return strpos($item, 'Awd_') !== false;
                }
            );
            $this->awdModules = $modules;
        }

        return $this->awdModules;
    }
    ...
    private function validateByExtension($extensions, $allModules = false)
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $allModules ? $this->getAllExtensions() : $this->getInstalledAwdExtensions();
    ...
    private function validateByNotInstalled($extensions)
    {
        if ($extensions) {
            ...
            if ($arrExtensions) {
                $installedModules = $this->getInstalledAwdExtensions();
     ...
     * @return bool
     */
    private function validateByAwdCount($counts)
    {
        $result = true;
        ...
        if ($countString) {
            ...
            $awdModules = $this->getInstalledAwdExtensions();
            $dependModules = $this->getDependModules($awdModules);
            $awdModules = array_diff($awdModules, $dependModules);

            $awdCount = count($awdModules);

            if ($awdCount
                && (
                    in_array($awdCount, $arrCounts, false) // non strict
                    || ($moreThan && $awdCount >= $moreThan)
                )
            ) {
                $result = true;
            }
     ...
     * @return array
     */
    private function getDependModules($awdModules)
    {
        $depend = [];
        $result = [];
        $dataName = [];
        foreach ($awdModules as $module) {
            ...
            if (isset($data['require']) && is_array($data['require'])) {
                foreach ($data['require'] as $requireItem => $version) {
                    if (strpos($requireItem, 'awd') !== false) {
                        $depend[] = $requireItem;
                    }
                }
            }
