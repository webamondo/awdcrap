...
namespace Awd\Base\Plugin\Backend\Model\Menu;
...
class Builder
{
    const BASE_MENU = 'MenuAwd_Base::menu';

    const SETTING_ENABLE = 'awd_base/menu/enable';
    ...
    private function cloneMenuItems($menuItems, Menu $menu)
    {
        ...
        foreach ($menuItems as $link) {
            $awdItem = $menu->get($link);
            if ($awdItem) {
                $itemData = $awdItem->toArray();
    ...
    {
        if ($this->awdItems === null) {
            $all = $this->generateAwdItems($menu);
            $this->awdItems = [];
            foreach ($all as $item) {
                $name = explode('::', $item);
                $name = $name[0];
                if (!isset($this->awdItems[$name])) {
                    $this->awdItems[$name] = [];
                }
                $this->awdItems[$name][] = $item;
            }
        }

        return $this->awdItems;
    }
     ...
     * @return array
     */
    private function generateAwdItems($menu)
    {
        $awd = [];
        foreach ($this->getMenuIterator($menu) as $menuItem) {
            ...
            if ($menuItem->hasChildren()) {
                foreach ($this->generateAwdItems($menuItem->getChildren()) as $menuChild) {
                    $awd[] = $menuChild;
                }
            }
        }
    ...
    {
        if (strpos($menuItem->getId(), 'Awd') === false
            || strpos($menuItem->getId(), 'Awd_Base') !== false) {
            return false;
        }
    ...
    private function getModuleTitle($name)
    {
        ...
        if ($module && isset($module['name'])) {
            ...
            $result = str_replace('Awd_', '', $result);
    ...
    private function generateConfigItems()
    {
        ...
        $config = $this->findResourceChildren($configTabs, 'awd');
