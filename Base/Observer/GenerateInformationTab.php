...
use Awd\Base\Plugin\Backend\Model\Menu\Item;
...
class GenerateInformationTab implements ObserverInterface
{
    ...
    const FEATURE_LINK = 'https://products.advancedwebsitedesign.co.uk/request-a-feature';
    ...
    {
        $src = $this->assetRepo->getUrl('Awd_Base::images/logo.png');
        if ($this->moduleInfoProvider->isOriginMarketplace()) {
            $href = Item::MAGENTO_MARKET_URL;
        } else {
            $href = 'https://advancedwebsitedesign.co.uk' . $this->getSeoParams() . 'awd_logo_' . $this->getModuleCode();
        }

        $html = '<a target="_blank" href="' . $href . '"><img class="awd-logo" src="' . $src . '"/></a>';
        ...
        if ($content) {
            ...
            foreach ($content as $message) {
                if (isset($message['type'], $message['text'])) {
                    $html .= '<div class="awd-additional-content"><span class="message ' . $message['type'] . '">'
                        . $message['text']
                        . '</span></div>';
                }
            }
    ...
    {
        $html = '<div class="awd-module-version">';
    ...
    private function showModuleExistingConflicts()
    {
        ...
        if (count($messages)) {
            $html = '<div class="awd-conflicts-title">'
                . __('Problems detected:')
                . '</div>';

            $html .= '<div class="awd-disable-extensions">';
